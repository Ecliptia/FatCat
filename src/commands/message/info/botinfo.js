...
export default class BotInfoMessage extends Command {
  ...
  async execute({ message, translate }) {
    logger.debug(
      `[Command:botinfo-message] Executando comando para ${message.author.tag}`,
    );
    try {
      ...
      const totalCommands =
        message.client.commands.size + message.client.slashCommands.size;
      ...
      if (filteredContributors.length > 0) {
        ...
        contributorsText += filteredContributors
          .map((c) => "**" + c.name + "**")
          .join(", ");
      } else {
      ...
      const initialEmbed = new EmbedBuilder()
        ...
        .setDescription(
          translate("botinfo.greeting") +
            ...
            "\n\n" +
            translate("botinfo.stats", {
              guilds: totalGuilds,
              users: totalUsers,
              commands: totalCommands,
              uptime: formattedUptime,
              shardId: sharderName[message.client.shard?.ids?.[0]] ?? 0,
              totalShards: message.client.shard?.count ?? 1,
            }) +
            "\n\n" +
      ...
      const buttonsRow = new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder()
          ...
          .setStyle(ButtonStyle.Link)
          .setURL(
            `https://discord.com/oauth2/authorize?client_id=${message.client.user.id}&permissions=8&scope=bot%20applications.commands`,
          ),
      );
      ...
      const collector = reply.createMessageComponentCollector({
        filter: (i) =>
          i.customId === "botinfo_select" && i.user.id === message.author.id,
      ...
      collector.on("collect", async (i) => {
        ...
        const heapTotalMemory = (memoryUsage.heapTotal / 1024 / 1024).toFixed(
          2,
        );
        ...
        switch (selected) {
          case "tech":
            newEmbed.setTitle(translate("botinfo.option_tech")).setDescription(
              translate("botinfo.tech_description", {
                djsVersion,
                moonlinkVersion,
                firebaseVersion,
                i18nextVersion,
          ...
          case "authors":
            newEmbed
              .setTitle(translate("botinfo.option_authors"))
              .setDescription(translate("botinfo.authors_description"));
            break;
          case "processing":
            newEmbed
              .setTitle(translate("botinfo.option_processing"))
              .setDescription(
                translate("botinfo.processing_description", {
                  rssMemory,
                  heapTotalMemory,
                  heapUsedMemory,
                }),
          ...
          case "database":
            newEmbed
              .setTitle(translate("botinfo.option_database"))
              .setDescription(translate("botinfo.database_description"));
            break;
          case "djs":
            newEmbed
              .setTitle(translate("botinfo.option_djs"))
              .setDescription(
                translate("botinfo.djs_description", { djsVersion }),
              );
            break;
          case "commands_executed":
            newEmbed
              .setTitle(translate("botinfo.option_commands_executed"))
              .setDescription(
                translate("botinfo.commands_executed_description", {
                  totalCommands,
                  messageCommands,
                  slashCommands,
                }),
            ...
            break;
          case "about_me":
            const createdAt = i.client.user.createdAt.toLocaleDateString(
              "pt-BR",
            ...
            const botStatus =
              i.client.user.presence?.status ||
              translate("botinfo.presence_unknown");
            const botActivity = i.client.user.presence?.activities[0]?.name
              ? `(${i.client.user.presence.activities[0].type} ${i.client.user.presence.activities[0].name})`
              : "";
            newEmbed
              .setTitle(translate("botinfo.option_about_me"))
              .setDescription(
                translate("botinfo.about_me_detailed", {
                  createdAt,
                  botId,
                  botTag,
                  botStatus,
      ...
      logger.debug(
        `[Command:botinfo-message] Respondeu para ${message.author.tag} com informações do bot.`,
      );
    } catch (error) {
      logger.error(
        `[Command:botinfo-message] Falha ao executar comando para ${message.author.tag}`,
        error,
      );
      await message.reply(
        translate(
