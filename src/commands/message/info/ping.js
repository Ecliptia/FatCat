    ...
    async execute({ message, translate }) {
        ...
        try {
            ...
            try {
                ...
                databaseLatency = Math.round(Number(dbEndTime - dbStartTime) / 1_000_000);
            } catch (dbError) {
                logger.error(`[Command:ping-message] Falha ao obter latência do banco de dados:`, dbError);
            ...
            logger.debug(`[Command:ping-message] Respondeu para ${message.author.tag} com latência: ${latency}ms, Latência API: ${apiLatency}ms, Latência DB: ${databaseLatency}ms`);
        } catch (error) {
