...
export default class BotInfoSlash extends Command {
    ...
    async execute({ interaction, translate }) {
        logger.debug(
            `[Command:botinfo-slash] Executando comando para ${interaction.user.tag}`,
        );
        try {
            ...
            const totalCommands =
                ...
                interaction.client.slashCommands.size;
            ...
            const initialEmbed = new EmbedBuilder()
                ...
                .setDescription(
                    ...
                    translate("botinfo.stats", {
                        ...
                        shardId: sharderName[interaction.client.shard?.ids?.[0]] ?? 0,
                        totalShards: interaction.client.shard?.count ?? 1,
                    }) +
                    "\n\n" +
            ...
            const collector = reply.createMessageComponentCollector({
                filter: (i) =>
                    i.customId === "botinfo_select" && i.user.id === interaction.user.id,
                time: 120000,
            });
            ...
            logger.debug(
                `[Command:botinfo-slash] Respondeu para ${interaction.user.tag} com informações do bot.`,
            );
        } catch (error) {
            logger.error(
                `[Command:botinfo-slash] Falha ao executar comando para ${interaction.user.tag}`,
                error,
            );
            await interaction.reply({
                content: translate(interaction.locale, "error.generic_error", {
                    emoji: "❌",
                }),
            });
        }
