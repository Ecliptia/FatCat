...
async function getGuildLocale(guildId) {
  ...
  try {
    ...
    logger.error(
      `[GuildSettings] Falha ao buscar configurações para a guilda ${guildId} no Firebase:`,
      error,
    );
    return "pt-BR";
  }
...
async function setGuildLocale(guildId, locale) {
  ...
  try {
    ...
    logger.debug(
      `[GuildSettings] Idioma da guilda ${guildId} definido para ${locale} no Firebase.`,
    );
  } catch (error) {
    logger.error(
      `[GuildSettings] Falha ao salvar configurações para a guilda ${guildId} no Firebase:`,
      error,
    );
  }
}
