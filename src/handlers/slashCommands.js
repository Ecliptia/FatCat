...
export default async function loadSlashCommands(client) {
  ...
  logger.debug(
    "[SlashCommands] Iniciando o carregamento dos comandos slash...",
  );

  const categoryDirs = await readdir(slashCommandsPath, {
    withFileTypes: true,
  });
    ...
    for (const commandItem of commandItems) {
      ...
      if (commandItem.isDirectory()) {
        ...
        const subcommandFiles = (
          await readdir(commandPath, { withFileTypes: true })
        ).filter((f) => f.isFile() && f.name.endsWith(".js"));

        if (subcommandFiles.length === 0) {
          logger.warn(
            `[SlashCommands] Diretório do comando ${commandName} está vazio, ignorando.`,
          );
          continue;
        }
        ...
        for (const subcommandFile of subcommandFiles) {
          ...
          try {
            const SubcommandClass = (
              await import(pathToFileURL(subcommandPath).href)
            ).default;
            ...
            logger.error(
              `[SlashCommands] Falha ao carregar subcomando do arquivo ${subcommandPath}`,
              error,
            );
          }
        }
        ...
        if (subcommands.size > 0) {
          ...
          mainCommand.execute = async function ({ interaction, translate }) {
            ...
            if (subcommand) {
              logger.debug(
                `[SlashCommands] Executando subcomando: ${subcommandName} do comando ${this.name}`,
              );
              await subcommand.execute({ interaction, translate });
            } else {
              logger.warn(
                `[SlashCommands] Subcomando não encontrado: ${subcommandName} para o comando ${this.name}`,
              );
            }
          };
          ...
          logger.debug(
            `[SlashCommands] Grupo de comandos carregado: ${mainCommand.name} com subcomandos: ${[...subcommands.keys()].join(", ")}`,
          );
        }
      } else if (commandItem.isFile() && commandItem.name.endsWith(".js")) {
        try {
          const CommandClass = (await import(pathToFileURL(commandPath).href))
            .default;
          ...
          if (command.slash) {
            ...
            logger.debug(
              `[SlashCommands] Comando slash carregado: ${command.name}`,
            );
          } else {
            logger.warn(
              `[SlashCommands] Ignorando comando de mensagem encontrado na pasta de comandos slash: ${command.name}`,
            );
          }
        } catch (error) {
          logger.error(
            `[SlashCommands] Falha ao carregar comando slash do arquivo ${commandPath}`,
            error,
          );
        }
      }
